...
|---|---|---|---|---|---|
| **DB schema + migrations** | `packages/db/prisma/schema.prisma` (+ `prisma/migrations/`) | rat145 | `feature/recruiter-post-a-job` | 2026-07-09 | Post a Job P3: `enum JobType` + `model Locality`; additive nullable `Job.jobType`/`openings`/`qualifications`/`localityId`/`internshipDurationMonths` + `City.localities` back-rel. One named migration. |
