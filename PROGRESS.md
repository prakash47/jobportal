...
- **Test counts on develop**: **580 API** + 181 web + 37 feature-flags + 19 observability + 17 auth = ~834 unit tests (recruiter-billing added 67 API tests: `recruiter-billing.service` 33 + `dto` 8 + `gst`/invoice-number 11 + `razorpay.client` 6 + `invoice-pdf` 2 + `tier-resolver` 5, plus the post-quota mock updated in place).
