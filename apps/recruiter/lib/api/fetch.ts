...
//
// KEPT IDENTICAL to apps/web/lib/api/fetch.ts and the other app's copy. Three
// copies exist because there is no shared browser-only package to hold it:
// packages/auth has no subpath exports, so a 'use client' module there would be
// pulled into the NestJS server bundle through its barrel. Consolidating this
// into a package is a recorded follow-up. Each app ships the SAME test file, so
// if these three drift, a test fails rather than a session silently breaking.
//
