...
interface RevealCodeButtonProps {
  challengeId: number;
  /** "email" / "mobile" — used to make the control self-describing out of context. */
  channelNoun: string;
...
export function RevealCodeButton({
  ...
  async function reveal(): Promise<void> {
    ...
    try {
      ...
      // "live" is as old as the tab. The response carries the two facts that can
      // have changed since, and both are re-checked HERE, against the browser's
      // clock at click time. Withholding the digits is a UI decision, not a
      // secrecy boundary — the code is already in this response either way — but
      // reading out a dead code costs a phone call and a retry, which is the
      // precise failure this surface exists to avoid.
      //
  ...
  // what would happen if this span only appeared once a code arrived. Revealing
  // also unmounts the button, dropping keyboard focus back to the document, and
  // the announcement is what tells a screen-reader user the digits arrived
  // rather than that a control simply vanished.
  return (
      ...
      <span role="status" className="block">
        {outcome === null ? (
          ...
          <span className="block">
            ...
            {/* Absolute, never a countdown. This tab may sit open for an hour,
                and nothing here re-renders on a timer, so a relative figure
                computed once at reveal would still read "in 14 minutes" long
                after the code died. */}
