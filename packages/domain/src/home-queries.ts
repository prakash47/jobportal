...
// Deliberately UNCACHED here. The web app wraps this in React's `cache()`
// for request-scoped dedup (apps/web/lib/home/queries.ts); that helper is
// RSC-only and is inert outside a React request, so it cannot live in a
// package shared with NestJS. A server that wants caching adds its own.
